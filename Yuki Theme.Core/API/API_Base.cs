...
namespace Yuki_Theme.Core.API
{
	public abstract class API_Base
	{
		...
		public void LoadSchemes (Func<string> ifZero = null)
		{
			ClearThemeInfos ();
			DefaultThemes.Clear ();
		...
		public void Save (Image wallpaper = null, Image sticker = null, bool wantToKeep = false)
		{
			Helper.CreateThemeDirectory ();
			if (!IsDefault ())
			{
		...
		/// <param name="setTheme">After theme has been set. You can use it to apply changes</param>
		public virtual void Preview (PreviewOptions options, Action setTheme = null)
		{
			string path = Path.Combine (Settings.pascalPath, "Highlighting", $"{pathToLoad}.xshd");
			...
			if (options.Syntax != SyntaxType.NULL)
			{
				...
				_actions.MergeSyntax (dir, options.Syntax);
			} else
			{
				PrepareToExport (path);
			}
		...
		public void ImportTheme (string path, Func<string, string, bool> exist)
		{
		...
		public int RenameTheme (string from, string to)
		{
		...
		public bool IsPascalDirectory (string path)
		{
		...
		public string GetNameOfTheme (string path)
		{
		...
		public string Translate (string key, string p1)
		{
			return Settings.translation.GetText (key).Replace ("{0}", p1);
		}

		public string Translate (string key, string p1, string p2)
		{
		...
		public Theme GetTheme (string name) => _themeManager.GetTheme (name);

		public bool IsDefault () => _actions.IsDefault ();
		...
		public bool IsOldTheme (string path) => _actions.IsOldTheme (path);

		public void ExtractSyntaxTemplate (SyntaxType syntax, string destination) =>
		...
		public void AddThemeInfo (string name, ThemeInfo themeInfo) => _actions.AddThemeInfo (name, themeInfo);

		#endregion

		private void ClearThemeInfos ()
		{
			_schemes.Clear ();
			_themeInfos.Clear ();
		}
		...
		private void IfThemesNotFound (Func<string> ifZero)
		{
			if (_schemes.Count == 0)
			{
				if (ifZero != null)
		...
		public void ShowError (bool canShowError, string message, string title)
		{
