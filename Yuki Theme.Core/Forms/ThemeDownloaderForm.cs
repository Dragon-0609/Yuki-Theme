...
namespace Yuki_Theme.Core.Forms
{
	...
	public partial class ThemeDownloaderForm : Form
	{
		...
		{
			string message = API.CentralAPI.Current.Translate ("theme.downloader.errors.group.request");
			string wrong = API.CentralAPI.Current.Translate ("theme.downloader.errors.group.wrong");
		...
		private void DownloadThemeThread (object param)
		{
			...
			if (notEqual)
			{
				...
				try
				{
					...
					{
						if (API.CentralAPI.Current.ThemeInfos [name].location == ThemeLocation.File &&
							File.Exists (PathGenerator.PathToFile (Helper.ConvertNameToPath (name), API.CentralAPI.Current.ThemeInfos [name].isOld)))
						{
							File.Delete (PathGenerator.PathToFile (Helper.ConvertNameToPath (name), API.CentralAPI.Current.ThemeInfos [name].isOld));
						}
					}
					...
					API.CentralAPI.Current.ExtractSyntaxTemplate (SyntaxType.Pascal, theme.fullPath); // Create theme file
					API.CentralAPI.Current.SaveTheme (theme, wallpaper, sticker);
					wallpaper.Dispose ();
					sticker.Dispose ();
					AddNSetProgress ();
