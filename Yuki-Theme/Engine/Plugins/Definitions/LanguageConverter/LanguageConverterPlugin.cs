...
{
	public class LanguageConverter_ : IVisualPascalABCPlugin
	{
		...
		public TextFormatterForm TextFormatterForm;
		//public TextFormatter TextFormatter;
		// public List<ISemanticNodeConverter> Languages;

		public string Name
		{
			get { return "Language Converter"; }
		}
		...
		public LanguageConverter_(IWorkbench Workbench)
		{
		...
		public void ConvertSourceText()
		{
			try
			{
				/*PascalABCCompiler.SemanticTree.IProgramNode Root = VisualEnvironmentCompiler.StandartCompiler
					.SemanticTree;
				//SemanticNodeConverter semanticNodeConverter = new SemanticNodeConverter(new ListSyntRules(
				//    System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().ManifestModule.FullyQualifiedName) + "\\Rules_CS.txt"), TextFormatter/*new TextFormatter()#1#);
				...
				TextFormatterForm.SaveTextFormater(currentLanguage.SourceTextBuilder.TextFormatter);
				SemanticTreeVisitor
					Visitor = new SemanticTreeVisitor(
						currentLanguage); //new SemanticNodeConverter(new ListSyntRules("Rules_CS.txt"), new TextFormatter()));
				Root.visit(Visitor);
			...
			catch (NotSupportedException e)
			{
				System.Windows.Forms.MessageBox.Show(string.Format(
					"Извините, генерация кода для узла {0} не поддерживается. Текст не был сгенерирован.", e.Message));
			}
			catch (Exception e)
			{
				VisualEnvironmentCompiler.ExecuteAction(VisualEnvironmentCompilerAction.AddTextToCompilerMessages,
					"LanguageConverter Exception: " + e.ToString());
			}
		}
		...
		/*
		void Compiler_OnChangeCompilerState(PascalABCCompiler.ICompiler sender, PascalABCCompiler.CompilerState State,
			string FileName)
		{
		...
		private List<ISemanticNodeConverter> LoadLanguages()
		{
			...
			string dir = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly()
			...
			foreach (FileInfo fi in dllfiles)
			{
				...
				{
					if (asssembly != null &&
					    asssembly.FullName != System.Reflection.Assembly.GetExecutingAssembly().FullName)
					{
		...
		*/
		...
		{
			PluginGUIItem Item = new PluginGUIItem(StringsPrefix + "BUILD", StringsPrefix + "BUILD",
				TextFormatterForm.ImageBuild.Image, TextFormatterForm.ImageBuild.BackColor, ExecuteBuild);
			MenuItems.Add(Item);
			ToolBarItems.Add(Item);
			Item = new PluginGUIItem(StringsPrefix + "OPTIONS", StringsPrefix + "OPTIONS",
				TextFormatterForm.ImageOptions.Image, TextFormatterForm.ImageOptions.BackColor, ExecuteOptions);
			MenuItems.Add(Item);
			ToolBarItems.Add(Item);
		}
	}
