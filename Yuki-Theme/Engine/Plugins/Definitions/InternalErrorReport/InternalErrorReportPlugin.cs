...
{
	public class InternalErrorReport_ : IVisualPascalABCPlugin
	{
		public static string StringsPrefix = "VPP_IER_";
		IVisualEnvironmentCompiler VisualEnvironmentCompiler;
		public CompilerInternalErrorReport CompilerInternalErrorReport = new CompilerInternalErrorReport();
		public ErrorReport ErrorReport = new ErrorReport();
		private List<string> FileNames = new List<string>();
		private string ReportText = null;
		private string States = "";

		public InternalErrorReport_(IWorkbench Workbench)
		{
		...
		*/
		...
		private string GetInfo()
		{
			...
			/*if (VisualEnvironmentCompiler.Compiler.InternalDebug.DebugVersion)
				...
				s = ", relase version";
			return string.Format("{0} ({1}){2}{3}", PascalABCCompiler.Compiler.Banner,
				       PascalABCCompiler.Compiler.VersionDateTime.ToShortDateString(), s, Environment.NewLine) +
					...
					foreach (PascalABCCompiler.Errors.Error error in VisualEnvironmentCompiler.Compiler.ErrorsList)
						if (error is PascalABCCompiler.Errors.CompilerInternalError)
						{
							...
							for (int i = 0; i < VisualEnvironmentCompiler.Compiler.ErrorsList.Count; i++)
								ReportText += string.Format("Error[{0}]: {1}{2}", i,
									VisualEnvironmentCompiler.Compiler.ErrorsList[i].ToString(), Environment.NewLine);
		...
		public string Name
		{
			get { return "InternalError Report Genegator"; }
		}
		...
		public void GetGUI(List<IPluginGUIItem> MenuItems, List<IPluginGUIItem> ToolBarItems)
		{
			PluginGUIItem Item = new PluginGUIItem(StringsPrefix + "NAME", StringsPrefix + "DESCRIPTION",
				ErrorReport.PluginImage.Image, ErrorReport.PluginImage.BackColor, Execute);
			MenuItems.Add(Item);
		}
	}
}