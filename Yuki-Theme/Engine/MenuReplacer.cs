...
using SvgRenderer = YukiTheme.Tools.SvgRenderer;
...
public class MenuReplacer
{
    ...
    private Dictionary<string, Image> _baseIcons = new();
    ...
    private Dictionary<string, Func<Image, Image>> _getUpdatedIcon = new();
    ...
    {
        Image image = LoadMenuIcon(icon, ColorReference.BorderColor);
        if (canBeUpdated)
        {
            image = _getUpdatedIcon[icon](image);
        }
    ...
    private Image LoadMenuIcon(string icon, Color color = default)
    {
        ...
        SvgDocument document = SvgRenderer.LoadSvg(icon, "Icons.");
        SvgRenderInfo context = new SvgRenderInfo(document, false, Size.Empty, color != default, color == default ? Color.Red : color);
        Image image = SvgRenderer.RenderSvg(new Size(32, 32), context);
