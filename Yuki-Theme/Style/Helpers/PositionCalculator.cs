...
public class PositionCalculator
{
	...
	private readonly StickerWindow window;
	...
	{
		var owner = window.GetOwnerRectangle();
	...
	{
		float res = 0;
		var style = window.AlignX;
		var left = Math.Abs(owner.Left - window.Left);
		if (style == AlignmentX.Left)
			res = (float)left;
		else if (style == AlignmentX.Center)
			res = (float)(point.X - (point.X - left) - width);
		else if (style == AlignmentX.Right) res = (float)(owner.Width - (left + window.Width));
	...
	{
		float res = 0;
		var style = window.AlignY;
		var top = Math.Abs(owner.Top - window.Top);
		if (style == AlignmentY.Top)
			res = (float)top;
		else if (style == AlignmentY.Center)
			res = (float)(point.Y - (point.Y - top) - height);
		else if (style == AlignmentY.Bottom) res = (float)(owner.Height - (top + window.Height));
