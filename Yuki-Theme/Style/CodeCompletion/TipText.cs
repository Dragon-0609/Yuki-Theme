...
namespace YukiTheme.Style.CodeCompletion
{
	class CountTipText: TipText
	{
		...
		void DrawTriangle(float x, float y, bool flipped)
		{
			...
			base.Graphics.FillRectangle(ColorReference.BackgroundBrush, new RectangleF(x, y, triHeight, triHeight));
			float triHeight2 = triHeight / 2;
			float triHeight4 = triHeight / 4;
			Brush brush = ColorReference.ForegroundBrush; //Brushes.Black;
			if (flipped) {
