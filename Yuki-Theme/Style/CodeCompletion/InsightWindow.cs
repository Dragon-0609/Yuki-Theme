...
{
	public class YukiInsightWindow : AbstractCompletionWindow
	{
		public YukiInsightWindow(Form parentForm, TextEditorControl control) : base(parentForm, control)
		{
		...
		internal Point GetCusorCoord()
		{
			...
			int ypos = (control.ActiveTextAreaControl.Document.GetVisibleLine(caretPos.Y) + 1) *
			           control.ActiveTextAreaControl.TextArea.TextView.FontHeight
			           - control.ActiveTextAreaControl.TextArea.VirtualTop.Y;
			int rulerHeight = control.TextEditorProperties.ShowHorizontalRuler
				? control.ActiveTextAreaControl.TextArea.TextView.FontHeight
				: 0;
		...
		protected override void OnPaint(PaintEventArgs pe)
		{
			...
			else
			{
				...
				{
					methodCountMessage =
						VisualPascalABC.Tools.GetRangeDescription(CurrentData + 1, DataProvider.InsightDataCount);
				}
			...
			int num_param = PseudoDataProvider.Get<int>(DataProvider, "num_param");
		...
		{
			get { return insightDataProviderStack.Count; }
		}
