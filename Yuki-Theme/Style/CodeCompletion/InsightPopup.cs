...
public class InsightPopup : PABCNETInsightWindow
{
	...
	{
		return this.GetPropertyByReflection<IInsightDataProvider>("DataProvider");
	}

	private int CurrentData()
	{
		return this.GetPropertyByReflection<int>("CurrentData");
	}
	...
	protected override void OnPaint(PaintEventArgs pe)
	{
		...
		{
			if (dataProvider.InsightDataCount > 1) methodCountMessage = VisualPascalABC.Tools.GetRangeDescription(CurrentData() + 1, dataProvider.InsightDataCount);
		...
		var num_param = dataProvider.GetByReflection<int>("num_param", true);
