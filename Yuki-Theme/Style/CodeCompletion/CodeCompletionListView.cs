...
namespace YukiTheme.Style.CodeCompletion
{
	...
	/// </summary>
	public class YukiCodeCompletionListView : System.Windows.Forms.UserControl
	{
		...
		{
			get { return Math.Max(imageList.ImageSize.Height, (int)(Font.Height * 1.1)); }
		}
			...
			foreach (ICompletionData data in compData)
			{
				if (data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider.IconNumberLocal)
					...
					consts.Add(data);
				         ...
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberInternalMethod ||
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberPrivateMethod ||
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberProtectedMethod)
				{
					if (!data.Description.Contains("(" +
						...
						extension_meths.Add(data);
				         ...
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberInternalProperty ||
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberPrivateProperty ||
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberProtectedProperty)
					props.Add(data);
				else if (data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider.IconNumberField ||
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberInternalField ||
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberPrivateField ||
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberProtectedField)
					fields.Add(data);
				else if (data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider.IconNumberEvent ||
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberInternalEvent ||
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberPrivateEvent ||
				         data.ImageIndex == VisualPascalABC.CodeCompletionProvider.ImagesProvider
					         .IconNumberProtectedEvent)
					events.Add(data);
		...
		protected override void OnPaint(PaintEventArgs pe)
		{
			...
			while (curItem < completionData.Length && yPos < Height)
			{
				...
				if (drawingBackground.IntersectsWith(pe.ClipRectangle))
				{
					...
					RectangleF drawingBackgroundText =
						new RectangleF(imageExists ? imageWidth + 3 : 1, yPos, Width - 2, itemHeight);
					// draw Background
					if (curItem == selectedItem)
					{
						g.FillRectangle(ColorReference.BackgroundBrush, drawingBackground);
						g.FillRectangle(ColorReference.SelectionBrush, drawingBackgroundText);
					...
					if (curItem == selectedItem)
					{
						g.DrawString(completionData[curItem].Text, Font, ColorReference.ForegroundBrush, xPos,
							yPos + dfont);
					}
					else
					{
