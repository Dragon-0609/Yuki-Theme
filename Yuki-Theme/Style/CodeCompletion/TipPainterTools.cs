            ...
            if (IsVisibleText(description))
            {
                ...
                if (splitDescription.Length > 0)
                {
                    ...
                    if (param_num == 1)
                    {
                        ...
                        if (bold_beg != 0)
                        {
                            int end = basicDescription.IndexOf(global::CodeCompletion.CodeCompletionController.CurrentParser.LanguageInformation.ParameterDelimiter);
                    ...
                    else
                    {
                        ...
                        while (i < param_num)
                        {
                            bold_beg = basicDescription.IndexOf(global::CodeCompletion.CodeCompletionController.CurrentParser.LanguageInformation.ParameterDelimiter, bold_beg) + 1;
                            if (bold_beg == 0) break;
                            i++;
                        }
                        if (bold_beg != 0)
                        {
                            int end = basicDescription.IndexOf(global::CodeCompletion.CodeCompletionController.CurrentParser.LanguageInformation.ParameterDelimiter, bold_beg);
                            int end_sk = basicDescription.IndexOf(')', bold_beg);
                            if (end > end_sk || end == -1) end = end_sk;
                            if (end != -1) bold_len = end - bold_beg;
                        }
