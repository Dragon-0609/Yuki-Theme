...
namespace YukiTheme.Style.CodeCompletion
{
	public class CodeCompletionHighlighter
	{
		...
		public static void Highlight(TextArea textArea)
		{
			try
			{
				Color color = ColorReference.TypeColor;
				int beg_off = -1;
				int end_off = -1;
				...
				if (string.Compare(word, "begin", true) == 0 || string.Compare(word, "repeat", true) == 0 ||
				    highlighted_keywords[word] != null)
				{
					...
					if (end_pos != null)
					{
						...
						TextMarker marker = new TextMarker(beg_off, word.Length, TextMarkerType.SolidBlock, color);
						marker.WholeLine = false;
						textArea.Document.MarkerStrategy.AddMarker(marker);
						marks.Add(marker);
						textArea.Document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.SingleLine,
							textArea.Document.GetLineNumberForOffset(beg_off)));
						textArea.EndUpdate();
						...
						marker = new TextMarker(end_pos.start_pos, end_pos.len, TextMarkerType.SolidBlock, color);
						marker.WholeLine = false;
						textArea.Document.MarkerStrategy.AddMarker(marker);
						marks.Add(marker);
						textArea.Document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.SingleLine,
							textArea.Document.GetLineNumberForOffset(end_pos.start_pos)));
						textArea.EndUpdate();
				...
				else if (string.Compare(word, "end", true) == 0 || string.Compare(word, "until", true) == 0)
				{
					...
					if (beg_pos != null)
					{
						...
						marker = new TextMarker(beg_pos.start_pos, beg_pos.len, TextMarkerType.SolidBlock, color);
						marker.WholeLine = false;
						textArea.Document.MarkerStrategy.AddMarker(marker);
						marks.Add(marker);
						textArea.Document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.SingleLine,
							textArea.Document.GetLineNumberForOffset(beg_pos.start_pos)));
						textArea.EndUpdate();
						textArea.Document.CommitUpdate();
					}
				}
			...
			catch (Exception e)
			{
				...
				File.AppendAllText("log.txt",
					DateTime.Now + Environment.NewLine + e.Message + Environment.NewLine + e.StackTrace +
					Environment.NewLine);
#endif
			}
		}
			...
			if (marks != null)
			{
				foreach (TextMarker marker in marks)
				{
					try
					{
						...
						textArea.Document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.SingleLine,
							textArea.Document.GetLineNumberForOffset(marker.Offset)));
						textArea.EndUpdate();
					}
					catch
					{
			...
			while (end_off < text.Length)
			{
				...
				else
				{
					...
					else if (string.Compare(word, "end", true) == 0)
					{
						...
						{
							string s = beg_stack.Pop();
							if (beg_stack.Count == 0 &&
							    (string.Compare(s, "begin", true) == 0 || highlighted_keywords[s] != null))
		...
		private static TmpPos GetPositionOfBegin(TextArea textArea, int beg_off)
		{
			...
			while (beg_off >= 0)
			{
				...
				else
				{
					...
					else if (string.Compare(word, "begin", true) == 0 || highlighted_keywords[word] != null ||
					         ignored_keywords[word] != null)
					{
						if (!CheckForCommentOrKavAhead(text, beg_off))
						{
							...
							beg_stack.Pop();
							if (beg_stack.Count == 0 && (string.Compare(word, "begin", true) == 0 ||
							                             highlighted_keywords[word] != null))
							{
		...
		private static string GetWordAtOffset(TextArea textArea, out int beg_off, out int end_off)
		{
			...
			StringBuilder sb = new StringBuilder();
			if (off < 0 || off >=
			    text.Length /*|| !(char.IsLetterOrDigit(text[off]) || text[off]=='&' || text[off]=='_')*/)
			{
				beg_off = -1;
				end_off = -1;
				return "";
			...
			if (off >= 1 &&
			    (off >= text.Length || !(char.IsLetterOrDigit(text[off]) || text[off] == '&' || text[off] == '_' ||
			                             text[off] == '.')) && (char.IsLetterOrDigit(text[off - 1]) ||
			                                                    text[off - 1] == '&' || text[off - 1] == '_' ||
			                                                    text[off - 1] == '.'))
				off--;
			...
			while (off >= 0 && off < text.Length && (char.IsLetterOrDigit(text[off]) || text[off] == '&' ||
			                                         text[off] == '_' || text[off] == '.'))
			{
				off--;
			}
