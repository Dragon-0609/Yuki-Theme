...
namespace YukiTheme.Style.CodeCompletion
{
	public class YukiCodeCompletionWindow : AbstractCompletionWindow
	{
		...
		YukiCodeCompletionListView codeCompletionListView;
		...
		{
			get { return _is_overrided_meths; }
			set
			{
		...
		public static YukiCodeCompletionWindow ShowCompletionWindow(Form parent, TextEditorControl control,
			string fileName, ICompletionDataProvider completionDataProvider, char firstChar, bool visibleKeyPressed,
			bool is_by_dot, PascalABCCompiler.Parsers.KeywordKind keyw)
		{
			ICompletionData[] completionData =
				(completionDataProvider as VisualPascalABC.CodeCompletionProvider).GenerateCompletionDataWithKeyword(
					fileName, control.ActiveTextAreaControl.TextArea, firstChar, keyw);
			if (completionData == null || completionData.Length == 0)
			{
				return null;
			}

			YukiCodeCompletionWindow codeCompletionWindow = new YukiCodeCompletionWindow(completionDataProvider,
				completionData, parent, control, visibleKeyPressed, is_by_dot);
			codeCompletionWindow.ShowCompletionWindow();
			return codeCompletionWindow;
		}

		public static YukiCodeCompletionWindow ShowOverridableMethodsCompletionWindows(Form parent,
			TextEditorControl control, string fileName, ICompletionDataProvider completionDataProvider)
		{
			ICompletionData[] completionData =
				(completionDataProvider as VisualPascalABC.CodeCompletionProvider)
				.GenerateCompletionDataForOverridableMethods(fileName, control.ActiveTextAreaControl.TextArea);
			if (completionData == null || completionData.Length == 0)
			{
				return null;
			}
			...
			YukiCodeCompletionWindow codeCompletionWindow =
				new YukiCodeCompletionWindow(completionDataProvider, completionData, parent, control, false, false);
		...
		public static YukiCodeCompletionWindow ShowCompletionWindowWithFirstChar(Form parent,
			TextEditorControl control, string fileName, ICompletionDataProvider completionDataProvider, char firstChar,
			PascalABCCompiler.Parsers.KeywordKind keyw)
		{
			ICompletionData[] completionData =
				(completionDataProvider as VisualPascalABC.CodeCompletionProvider).GenerateCompletionDataByFirstChar(
					fileName, control.ActiveTextAreaControl.TextArea, firstChar, keyw);
			if (completionData == null || completionData.Length == 0)
			{
				return null;
			...
			YukiCodeCompletionWindow codeCompletionWindow =
				new YukiCodeCompletionWindow(completionDataProvider, completionData, parent, control, true, false);
			codeCompletionWindow.ShowCompletionWindow();
			return codeCompletionWindow;
		}

		YukiCodeCompletionWindow(ICompletionDataProvider completionDataProvider, ICompletionData[] completionData,
			Form parentForm, TextEditorControl control, bool visibleKeyPressed, bool is_by_dot) : base(parentForm,
			control)
		{
			...
			codeCompletionListView.Font =
				new Font(VisualPascalABC.Constants.CompletionWindowCodeCompletionListViewFontName,
			...
			float proportion = MiscHelper.InverseLerp(8, 20, CompletionFontSizeFinder.GetSize());
			float additionalSize = MiscHelper.Lerp(1, 1.8f, proportion);
			this.drawingSize = new Size( //codeCompletionListView.ItemHeight * 10,
				(int)(VisualPascalABC.Constants.CompletionWindowWidth * additionalSize),
			...
			if (declarationViewWindow == null)
			{
				...
				declarationViewWindow.Font =
					new Font(VisualPascalABC.Constants.CompletionWindowDeclarationViewWindowFontName,
						CompletionFontSizeFinder.GetSize());
			}
			...
			if ((completionDataProvider as VisualPascalABC.CodeCompletionProvider).DefaultCompletionElement != null)
			{
				...
				codeCompletionListView.SelectIndexByCompletionData(
					(completionDataProvider as VisualPascalABC.CodeCompletionProvider).DefaultCompletionElement);
			}
		...
		public void HandleMouseWheel(MouseEventArgs e)
		{
			...
			if (System.Windows.Forms.SystemInformation.MouseWheelScrollLines > 0)
			{
				newValue = this.vScrollBar.Value -
				           (control.TextEditorProperties.MouseWheelScrollDown ? 1 : -1) * multiplier;
			}
			else
			{
				newValue = this.vScrollBar.Value -
		...
		void fiexerrorTextAreaInvalidate()
		{
			...
			Rectangle r = new Rectangle(lastCursorScreenPosition,
				new Size(control.ActiveTextAreaControl.TextArea.Font.Height,
					control.ActiveTextAreaControl.TextArea.Font.Height + 3));
		...
		{
			Console.WriteLine($"Is visible: {Visible}");
			if (!Visible)
			{
				return false;
			}
		...
		{
			if (codeCompletionListView == null || Disposing || IsDisposed) return false;
			document.DocumentAboutToBeChanged -= DocumentAboutToBeChanged;
			ICompletionData data = codeCompletionListView.SelectedCompletionData;
