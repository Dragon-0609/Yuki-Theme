...
namespace Yuki_Theme.CLI
{
	public class ConditionalEdition
	{
		...
		private void CollectConditionsAndFields (IEnumerable <string> commands, out ConditionSet conditionSet)
		{
			...
			foreach (string commad in coms)
			{
				...
				if (isSetMode)
				{
					...
					{
						ShowError (CentralAPI.Current.Translate ("cli.errors.conditions.null"));
						conditionSet = null;
						return;
					}
			...
			{
				ShowError (CentralAPI.Current.Translate (conditions.Count > 0 ? "cli.errors.setter.null" : "cli.errors.conditions.null"));
				conditionSet = null;
			}
		}
		...
		private void CheckAndSet (ConditionSet set)
		{
			...
			foreach (KeyValuePair <string, ThemeInfo> pair in CentralAPI.Current.ThemeInfos)
			{
			...
			if (update.Count > 0)
			{
				...
				{
					CentralAPI.Current.ThemeInfos [pair.Key] = pair.Value;
				}
			}
		}
		...
		private void LoadIfNecessary (string name, Condition condition, ref Theme  theme, bool inCondition)
		{
			if (inCondition)
			{
				...
				if (need && theme == null)
				{
					theme = CentralAPI.Current.GetTheme (name);
				}
			}
		}
