...
namespace DokiNameNormalizer
{
    internal class Program
    {
        ...
        {
            Console.WriteLine("Specify type: 1 - Normalize Themes Name, 2 - Normalize Wallpaper and Sticker path, 3 - Normalize Wallpaper and sticker name");
        ...
        private static void Move(string[] files, string to)
        {
            foreach (string file in files)
            {
                string toFolder = Path.Combine(Directory.GetParent(file).FullName, to, $"{Path.GetDirectoryName(file)} {Path.GetFileName(file)}");
