...
namespace Yuki_Theme.Core.WPF.Windows
{
	internal class MainPresenter : Main.IPresenter
	{
		...
		public void LoadThemesToUi(ComboBox themeBox, ListView definitionsBox)
		{
			...
			_model.Themes = API.Schemes.ToArray ();
			...
			if (themeBox.Items.Contains (API.selectedItem))
				themeBox.SelectedItem = API.selectedItem;
			else
		...
		{
			MainParser.Parse (fileName, true, select, ErrorExport, AskChoiceParser, addToUiList, selectAfterParse);
		}
