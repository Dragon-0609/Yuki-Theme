...
namespace Yuki_Theme.Core.WPF.Windows
{
	public class UpdateDownloaderPresenter : UpdateDownloader.IPresenter
	{
		...
		private void AfterCheckingVersion (bool hasUpdate)
		{
			...
			if (hasUpdate)
			{
				title = API_Base.Current.Translate ("download.available");
				content = GetVersionFormatted (true);
				
				button1Data = new NotificationButtonData (API_Base.Current.Translate ("download.buttons.update"), true, StartUpdate);

				button2Data = new NotificationButtonData (API_Base.Current.Translate ("download.buttons.github"), _model.GetGithubUrl (), true,
					(sender, args) => { Process.Start (args.Uri.AbsoluteUri); });

			} else
			{
				title = API_Base.Current.Translate ("download.uptodate");
				content = API_Base.Current.Translate ("download.latest");
		...
		{
			return !withSize ? "" : $"{Environment.NewLine}{API_Base.Current.Translate ("download.size")}: {_model.GetSize ()}";
		}
		...
		private void DownloadingCompleted (object sender, AsyncCompletedEventArgs e)
		{
			if (e.Error != null)
			{
				string title = API_Base.Current.Translate ("download.refused.error");
				string message = e.Error.Message;
				if(e.Error.InnerException != null)
				{
					if (e.Error.InnerException is SocketException)
					{
						title = API_Base.Current.Translate ("download.refused.title");
						message = API_Base.Current.Translate ("download.refused.message");
					} else
					{
				...
				{
					NotificationButtonData button1Data = new (API_Base.Current.Translate ("download.buttons.github"), _model.GetGithubUrl (), true,
						(sender, args) => { Process.Start (args.Uri.AbsoluteUri); });
			...
			} else
			{
				if (e.Cancelled)
				{
					string title = API_Base.Current.Translate ("download.canceled.title");
					string content = API_Base.Current.Translate ("download.canceled.message");
				...
				} else
				{
					...
					questionWindow.Title = API_Base.Current.Translate ("download.downloaded.short");
					
					questionWindow.SetTitle (API_Base.Current.Translate ("download.downloaded.full"));
					questionWindow.SetButtonContents (API_Base.Current.Translate ("download.buttons.install"),
						API_Base.Current.Translate ("download.buttons.later"), "");
