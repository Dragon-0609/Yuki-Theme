...
namespace Yuki_Theme_Plugin
{
	public class ThemeSwitcher
	{
		...
		{
			if(!plugin.fm.Controls.ContainsKey ("Custom Panel Switcher"))
			{
				...
				plugin.RememberCurrentEditor ();
					...
					panel_bg.Size = plugin.fm.ClientSize;
					...
					plugin.fm.Controls.Add (panel_bg);
		...
		private void list_1_DrawItem (object sender, DrawItemEventArgs e)
		{
			...
			e.DrawBackground ();
			e.Graphics.DrawString (((ListBox) sender).Items [e.Index].ToString (), e.Font, YukiTheme_VisualPascalABCPlugin.clrBrush, e.Bounds);
		...
		internal void CloseOnClick (object sender, EventArgs e)
		{
			if (panel_bg != null)
			{
				...
				plugin.fm.Controls.Remove (panel_bg);
				panel_bg?.Dispose ();
		...
		private void PreviewTheme (string name, string oldName)
		{
			if(name != oldName)
			{
				if(CLI.SelectTheme (name))
				{
					...
					{
						SyntaxType type = ShadowNames.GetSyntaxByExtension (Path.GetExtension (plugin.fm.CurrentCodeFileDocument.FileName));
